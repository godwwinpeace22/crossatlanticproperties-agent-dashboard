...
export function ApprovalsList({ submissions }: ApprovalsListProps) {
  ...
  const handleApproval = async (submissionId: string, action: "approved" | "rejected") => {
    ...
    try {
      ...
      if (action === "approved") {
        ...
        const { error: hierarchyError } = await supabase.from("agent_hierarchy").insert([
          {
        ...
        await calculateCommissions(purchase.id, submitterProfile.id, Number.parseFloat(submission.amount))
      }
  ...
  const calculateCommissions = async (purchaseId: string, sellerId: string, amount: number) => {
    // Get commission settings
    const { data: commissionSettings } = await supabase.from("commission_settings").select("*").order("level")
    ...
    const commissionPromises = uplineChain.map(async (uplineAgent, index) => {
      const level = index + 1
      const commissionSetting = commissionSettings.find((cs) => cs.level === level)

      if (commissionSetting) {
        const commissionAmount = (amount * Number(commissionSetting.percentage)) / 100
  ...
  const getUplineChain = async (agentId: string): Promise<Array<{ id: string }>> => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (hierarchy?.upline_id) {
        uplineChain.push({ id: hierarchy.upline_id })
        currentAgentId = hierarchy.upline_id
      } else {
        break
      }
    }
                ...
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <CardDescription>
                        Submitted by: {submission.submitter.full_name || submission.submitter.email}
                      </CardDescription>
                    </div>
                ...
                <CardContent className="space-y-4">
                  <div className="grid gap-4 md:grid-cols-3">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{submission.buyer_email}</p>
                      </div>
                    </div>
                  ...
                  {submission.notes && (
                    <div className="p-3 bg-muted rounded-md">
                      <p className="text-sm text-muted-foreground mb-1">Submission Notes:</p>
                ...
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-lg">{submission.properties.name}</CardTitle>
                      <CardDescription>
                        {submission.buyer_name} - ${Number(submission.amount).toLocaleString()}
                      </CardDescription>
                    </div>
      ...
      {submissions.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center">All payment submissions have been processed.</p>
          </CardContent>
        </Card>
      )}
    </div>
